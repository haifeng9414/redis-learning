...
public class ArticleVote {
    ...
    public void articleVote(String userId, String article) {
        ...
        if (allKindOfBaseCommand.zscore("time:", article) < diff) {
            return;
        }
        ...
        if (allKindOfBaseCommand.sadd("voted:" + articleId, userId) > 0) {
            // 为文章增加分数
            allKindOfBaseCommand.zincrby("score:", VOTE_SCORE, article);
            // key为article的hash保存了文章的信息，这里文章的投票数 + 1
            allKindOfBaseCommand.hincrBy(article, "votes", 1L);
        }
    }
}
